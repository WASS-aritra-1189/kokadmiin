...
function Page() {
  ...
  const load = async (p = page, search = q) => {
    setLoading(true);
    try {
      const res = await productCategoryService.getAll({ page: p, limit: LIMIT, ...(search ? { search } : {}) });
  ...
  const handleStatusToggle = async (item: ProductCategory) => {
    const next = item.status === "ACTIVE" ? "DEACTIVE" : "ACTIVE";
    await productCategoryService.changeStatus(item.id, next);
    load();
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight">Product Categories</h1>
          <p className="mt-1 text-[13px] text-[#6B7280]">Manage product types like Books, Pens, Copies, Geometry Box, etc.</p>
        </div>
        <button
          ...
          <table className="w-full text-[12px]">
            ...
            <tbody>
              ...
              {!loading && items.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-[#6B7280]">No categories found.</td></tr>
              )}
              {!loading && items.map((item) => (
                <tr key={item.id} className="border-b border-[#F3F4F6] last:border-0 hover:bg-[#FAFAF9]">
                  <td className="px-4 py-2.5 font-medium">{item.name}</td>
                  <td className="px-4 py-2.5 text-[#6B7280] max-w-xs truncate">{item.description ?? "—"}</td>
                  <td className="px-4 py-2.5">
                    <button
                      onClick={() => handleStatusToggle(item)}
                      className={"rounded-full px-2 py-0.5 text-[10px] font-medium transition-colors " +
                        (item.status === "ACTIVE" ? "bg-[#DCFCE7] text-[#166534] hover:bg-[#BBF7D0]" : item.status === "DELETED" ? "bg-[#FEE2E2] text-[#991B1B]" : "bg-[#F3F4F6] text-[#4B5563] hover:bg-[#E5E7EB]")}
                    >
                      {item.status}
                    </button>
                  </td>
...
function CategorySheet({ item, onClose, onSaved }: { item: ProductCategory | null; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  ...
  const set = (k: keyof ProductCategoryPayload, v: string) => setForm((f) => ({ ...f, [k]: v }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await productCategoryService.update(item.id, form);
      } else {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#6B7280]">{isEdit ? "Edit category" : "New category"}</div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Books, Pens, Copies"
                className="h-8 w-full rounded-md border border-[#E5E7EB] bg-white px-2 text-[12px] outline-none focus:border-[#4F46E5]"
              />
            </div>
