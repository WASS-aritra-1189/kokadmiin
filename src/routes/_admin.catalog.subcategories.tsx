...
import { subCategoryService, type SubCategory } from "@/services/catalog-extra.service";
import { api } from "@/lib/axios";

export const Route = createFileRoute("/_admin/catalog/subcategories")({ component: Page });
...
function Page() {
  ...
  const [sheet, setSheet] = useState<{ open: boolean; item: SubCategory | null }>({ open: false, item: null });
  const [confirmId, setConfirmId] = useState<string | null>(null);
  const [deleting, setDeleting] = useState(false);

  useEffect(() => {
    api.get("/categories").then((r) => setCategories(r.data?.data?.data ?? []));
  }, []);

  const load = async (p = page, search = q) => {
    setLoading(true);
    try {
      const res = await subCategoryService.getAll({ page: p, limit: LIMIT, ...(search ? { search } : {}) });
  ...
  const handleStatusToggle = async (item: SubCategory) => {
    await subCategoryService.changeStatus(item.id, item.status === "ACTIVE" ? "DEACTIVE" : "ACTIVE");
    load();
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight">Subcategories</h1>
          <p className="mt-1 text-[13px] text-[#6B7280]">Second-level groupings nested under a parent category.</p>
        </div>
        <button
      ...
      <div className="mt-5 grid grid-cols-3 gap-3">
        {[["Total", total], ["Active", items.filter(i => i.status === "ACTIVE").length], ["Deactive", items.filter(i => i.status !== "ACTIVE").length]].map(([l, v]) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody>
              ...
              {!loading && items.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-[#6B7280]">No subcategories found.</td></tr>}
              {!loading && items.map((item) => (
                <tr key={item.id} className="border-b border-[#F3F4F6] last:border-0 hover:bg-[#FAFAF9]">
                  <td className="px-4 py-2.5">
                    <div className="flex items-center gap-2">
                      {item.image && <img src={item.image} alt={item.name} className="h-7 w-7 rounded object-cover" />}
                      <span className="font-medium">{item.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-2.5">
                    {item.category ? (
                      <span className="rounded-md bg-[#EEF2FF] px-2 py-0.5 text-[11px] font-medium text-[#4F46E5]">{item.category.name}</span>
                    ) : "—"}
                  </td>
                  <td className="px-4 py-2.5">
                    <button
                  ...
                  <td className="px-4 py-2.5 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <button onClick={() => setConfirmId(item.id)} className="rounded-md border border-[#FEE2E2] px-2 py-1 text-[11px] font-medium text-[#EF4444] hover:bg-[#FEF2F2]">Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
...
function SubCategorySheet({ item, categories, onClose, onSaved }: {
  ...
  const isEdit = !!item;
  const [form, setForm] = useState({ name: item?.name ?? "", categoryId: item?.categoryId ?? "", desc: item?.desc ?? "", status: item?.status ?? "ACTIVE" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await subCategoryService.update(item.id, form);
      else await subCategoryService.create({ name: form.name, categoryId: form.categoryId || undefined, desc: form.desc || undefined });
      onSaved();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#6B7280]">{isEdit ? "Edit subcategory" : "New subcategory"}</div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              ...
              <input required value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Fiction" className={inp} />
            </div>
            <div>
              <label className="mb-1 block text-[11px] font-medium text-[#374151]">Parent category</label>
            ...
            <div>
              ...
              <textarea rows={3} value={form.desc} onChange={(e) => setForm(f => ({ ...f, desc: e.target.value }))} placeholder="Optional description…" className="w-full rounded-md border border-[#E5E7EB] bg-white px-2 py-1.5 text-[12px] outline-none focus:border-[#4F46E5]" />
            </div>
            {isEdit && (
              <div>
