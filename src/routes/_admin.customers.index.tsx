...
import { X, Search, Loader2, Heart, BookOpen } from "lucide-react";
import { customersService, wishlistService, type Customer, type Wishlist } from "@/services/customers.service";
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight">All customers</h1>
          <p className="mt-1 text-[13px] text-[#6B7280]">{total} customers</p>
        </div>
      </div>
              ...
              <tbody>
                {customers.map((c) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-[#4B5563] capitalize">{c.gender || "—"}</td>
                    <td className="px-3 py-2"><StatusChip status={c.status} /></td>
                    <td className="px-3 py-2 text-[#4B5563]">{c.createdAt ? new Date(c.createdAt).toLocaleDateString() : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function CustomerDrawer({ customer, onClose }: { customer: Customer; onClose: () => void }) {
  const [tab, setTab] = useState<"details" | "wishlist">("details");
  ...
  useEffect(() => {
    ...
    wishlistService
      ...
      .catch(() => setWlError("Failed to load wishlist."))
      .finally(() => setWlLoading(false));
  }, [tab, customer.accountId]);
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="flex h-full w-full max-w-lg flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2 text-[10px]">
                ...
                <span className="text-[#6B7280]">Since {customer.createdAt ? new Date(customer.createdAt).toLocaleDateString() : "—"}</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex border-b border-[#E5E7EB] px-5">
          {(["details", "wishlist"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={"mr-4 py-2.5 text-[12px] font-medium border-b-2 transition-colors " +
                (tab === t ? "border-[#4F46E5] text-[#4F46E5]" : "border-transparent text-[#6B7280] hover:text-[#111827]")}
            >
              {t === "details" ? "Details" : "Wishlist"}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {tab === "details" ? (
            <div className="space-y-3">
              <Row label="Login ID" value={customer.loginId} />
              <Row label="Email" value={customer.email || "—"} />
              <Row label="Gender" value={customer.gender || "—"} />
              <Row label="Date of Birth" value={customer.dateOfBirth ? new Date(customer.dateOfBirth).toLocaleDateString() : "—"} />
              <Row label="Active" value={customer.isActive ? "Yes" : "No"} />
              <Row label="Account ID" value={customer.accountId} />
              {customer.bio && <Row label="Bio" value={customer.bio} />}
            </div>
          ) : wlLoading ? (
            <div className="flex items-center justify-center py-16 text-[#6B7280]">
            ...
            <div className="flex items-center justify-center py-16 text-red-500 text-[12px]">{wlError}</div>
            ...
            <div className="space-y-4">
              <p className="text-[11px] text-[#6B7280]">{totalItems} item{totalItems !== 1 ? "s" : ""} across {wishlists.length} list{wishlists.length !== 1 ? "s" : ""}</p>
              {wishlists.map((wl) => (
                <div key={wl.id}>
                  <div className="flex items-center gap-1.5 mb-2">
                    <Heart className="h-3 w-3 text-[#4F46E5]" />
                    <span className="text-[11px] font-semibold text-[#374151]">{wl.name}</span>
                    {wl.isDefault && <span className="rounded-full bg-[#EEF2FF] px-1.5 py-0.5 text-[9px] font-medium text-[#4F46E5]">Default</span>}
                    <span className="ml-auto text-[10px] text-[#9CA3AF]">{wl.items.length} item{wl.items.length !== 1 ? "s" : ""}</span>
                  </div>
                      ...
                      <div key={item.id} className="flex items-start gap-3 rounded-lg border border-[#E5E7EB] p-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="mt-1 flex items-center gap-2">
                            ...
                            <span className={"ml-auto rounded-full px-1.5 py-0.5 text-[9px] font-medium " + (item.book.status === "ACTIVE" ? "bg-[#DCFCE7] text-[#166534]" : "bg-[#F3F4F6] text-[#4B5563]")}>
                              {item.book.status}
                            </span>
                          </div>
                        </div>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between border-b border-[#F3F4F6] pb-2 text-[12px]">
      <span className="text-[#6B7280]">{label}</span>
      <span className="font-medium text-[#111827] text-right max-w-[60%] break-all">{value}</span>
    </div>
  );
}
