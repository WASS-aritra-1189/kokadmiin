    ...
    <div className="p-6">
      <div className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-2 text-[12px]">
          <select className="h-8 rounded-md border border-[#E5E7EB] bg-white px-2 text-[12px]">
          ...
          <div className="divide-y divide-[#F3F4F6]">
            {low.map((b) => (
              <div key={b.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className={"tabular-nums text-[12px] font-semibold " + (b.stock === 0 ? "text-[#EF4444]" : "text-[#F59E0B]")}>
                  {b.stock}
                </div>
              </div>
            ))}
...
function Kpi({ label, value, delta, icon: Icon, tint, invertDelta }: { label: string; value: string; delta: number; icon: any; tint: string; invertDelta?: boolean }) {
  ...
  return (
    <div className="rounded-lg border border-[#E5E7EB] bg-white p-4">
      ...
      <div className={"mt-1 flex items-center gap-1 text-[11px] font-medium " + (positive ? "text-[#10B981]" : "text-[#EF4444]")}>
        {positive ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
        {Math.abs(delta)}%<span className="font-normal text-[#9CA3AF]">vs last period</span>
      </div>
    </div>
  );
}
