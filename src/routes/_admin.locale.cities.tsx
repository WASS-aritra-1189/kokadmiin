...
function Page() {
  ...
  return (
    <div className="p-6">
      <div className="text-[11px] font-medium uppercase tracking-wider text-[#6B7280]">Localization</div>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight">Cities</h1>
          <p className="mt-1 text-[13px] text-[#6B7280]">Cities with delivery metadata. Powers address autocomplete and shipping rules.</p>
        </div>
        <button
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-[#F3F4F6] px-3 py-2">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") { setPage(1); load(1, q, stateFilter); } }}
          ...
          <select
            ...
            className="h-8 rounded-md border border-[#E5E7EB] bg-white px-2 text-[12px] outline-none disabled:opacity-50"
          >
            <option value="">{countryFilter ? "All states" : "Select country first"}</option>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody>
              ...
              {!loading && items.length === 0 && <tr><td colSpan={6} className="px-4 py-8 text-center text-[#6B7280]">No cities found.</td></tr>}
              {!loading && items.map((item) => (
                <tr key={item.id} className="border-b border-[#F3F4F6] last:border-0 hover:bg-[#FAFAF9]">
                  <td className="px-4 py-2.5 font-medium">{item.name}</td>
                  <td className="px-4 py-2.5 text-[#6B7280]">{item.state?.name ?? "—"}</td>
                  <td className="px-4 py-2.5 text-[#6B7280]">{item.state?.country?.name ?? "—"}</td>
...
function CitySheet({ item, countries, onClose, onSaved }: { item: City | null; countries: Country[]; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [countryId, setCountryId] = useState(item?.state?.country?.id ?? "");
  const [states, setStates] = useState<State[]>([]);
  const [form, setForm] = useState({ name: item?.name ?? "", stateId: item?.stateId ?? "", status: item?.status ?? "ACTIVE" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await cityService.update(item.id, form);
      else await cityService.create({ name: form.name, stateId: form.stateId });
      onSaved();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#6B7280]">{isEdit ? "Edit city" : "New city"}</div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              <label className="mb-1 block text-[11px] font-medium text-[#374151]">City name *</label>
              <input required value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Mumbai" className={inp} />
            </div>
            <div>
              <label className="mb-1 block text-[11px] font-medium text-[#374151]">Country *</label>
              <select required value={countryId} onChange={(e) => setCountryId(e.target.value)} className={inp}>
            ...
            <div>
              ...
              <select required value={form.stateId} onChange={(e) => setForm(f => ({ ...f, stateId: e.target.value }))} disabled={!countryId} className={inp + (!countryId ? " opacity-50 cursor-not-allowed" : "")}>
