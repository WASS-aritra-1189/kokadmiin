...
function Page() {
  ...
  useEffect(() => { load(); }, [page, countryFilter]);

  const totalPages = Math.max(1, Math.ceil(total / LIMIT));

  const handleStatusToggle = async (item: State) => {
    await stateService.changeStatus(item.id, item.status === "ACTIVE" ? "DEACTIVE" : "ACTIVE");
    load();
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight">States</h1>
          <p className="mt-1 text-[13px] text-[#6B7280]">States and provinces used for addresses and shipping zones.</p>
        </div>
        <button
        ...
        >
          <Plus className="h-3.5 w-3.5" />New state
        </button>
      </div>
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-[#F3F4F6] px-3 py-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") { setPage(1); load(1, q, countryFilter); } }}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody>
              ...
              {!loading && items.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-[#6B7280]">No states found.</td></tr>}
              {!loading && items.map((item) => (
                <tr key={item.id} className="border-b border-[#F3F4F6] last:border-0 hover:bg-[#FAFAF9]">
                  <td className="px-4 py-2.5 font-medium">{item.name}</td>
                  <td className="px-4 py-2.5 text-[#6B7280]">{item.country?.name ?? "—"}</td>
...
function StateSheet({ item, countries, onClose, onSaved }: { item: State | null; countries: Country[]; onClose: () => void; onSaved: () => void }) {
  const isEdit = !!item;
  const [form, setForm] = useState({ name: item?.name ?? "", countryId: item?.countryId ?? "", status: item?.status ?? "ACTIVE" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      if (isEdit) await stateService.update(item.id, form);
      else await stateService.create({ name: form.name, countryId: form.countryId });
      onSaved();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30" onClick={onClose}>
      <div className="flex h-full w-full max-w-md flex-col bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-[#E5E7EB] px-5 py-3">
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[#6B7280]">{isEdit ? "Edit state" : "New state"}</div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div>
              <label className="mb-1 block text-[11px] font-medium text-[#374151]">State name *</label>
              <input required value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Maharashtra" className={inp} />
            </div>
            <div>
              <label className="mb-1 block text-[11px] font-medium text-[#374151]">Country *</label>
              <select required value={form.countryId} onChange={(e) => setForm(f => ({ ...f, countryId: e.target.value }))} className={inp}>
